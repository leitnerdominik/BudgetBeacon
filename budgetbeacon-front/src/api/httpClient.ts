...
import { notifyUnauthorized } from "../events/authEvents";

const localApiHostPattern = /^https?:\/\/(?:localhost|127\.0\.0\.1|\[::1\])(?::\d+)?$/i;
...
const resolveApiBaseUrl = () => {
  ...
  // Browser localhost points to the user's machine, so never bake it into
  // production bundles. Use the same-origin API path instead.
  if (import.meta.env.PROD && localApiHostPattern.test(normalizedBackendUrl)) {
    return "/api";
  }
...
export const INVALID_CSRF_TOKEN_TYPE = "urn:budgetbeacon:invalid-csrf-token";
