...
BudgetBeacon is a full-stack personal finance application with a React frontend and an ASP.NET Core backend. Users can sign in, upload transaction CSV files, browse their transaction history, inspect monthly summaries, and fetch AI-generated savings tips.
...
The frontend lives in `budgetbeacon-front/src` and is feature-oriented:
...
- EF Core uses PostgreSQL via `BudgetBeaconDbContext`.
...
```powershell
dotnet tool run dotnet-ef migrations list --project budgetbeacon-back\BudgetBeacon.Infrastructure\BudgetBeacon.Infrastructure.csproj --context BudgetBeacon.Infrastructure.Data.BudgetBeaconDbContext
dotnet tool run dotnet-ef database update --project budgetbeacon-back\BudgetBeacon.Infrastructure\BudgetBeacon.Infrastructure.csproj --context BudgetBeacon.Infrastructure.Data.BudgetBeaconDbContext
```
...
```powershell
$env:ConnectionStrings__DefaultConnection="Host=localhost;Port=5432;Database=budgetbeacon;Username=YOUR_USER;Password=YOUR_PASSWORD;SSL Mode=Disable"
...
- `budgetbeacon-front/dist`

### Backend

```powershell
cd budgetbeacon-back\BudgetBeacon.Api
dotnet build BudgetBeacon.Api.csproj -v minimal -p:UseAppHost=false
```
...
```powershell
cd budgetbeacon-back\BudgetBeacon.Api
dotnet publish BudgetBeacon.Api.csproj -c Release -o .\publish
```

The publish output is written to:

- `budgetbeacon-back/BudgetBeacon.Api/publish`

## Deployment overview

The application is not fully production-ready yet, but the intended deployment shape is:

1. Build the frontend and host `budgetbeacon-front/dist` behind a static web server or reverse proxy.
